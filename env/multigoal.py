...
class MultiGoalEnv(gym.Env):
  def __init__(self):
    self.threshold = 0.5
    self.r_threshold = 0.5
    self.r_coeff = 10.
    self.g_coeff = 1.
    self.c_coeff = 1e-2
    self.live_cost = 0.
    self.action_space = Box(-1, 1, (2,), dtype=np.float32)
    self.observation_space = Box(-9, 9, (2,), dtype=np.float32)
    self.p = np.array([0, 0], dtype=np.float32)
    self.gs = np.array([
        [6,0], [0,6],
        [-6,0], [0,-6]], dtype=np.float32)
    self.info = {}
    self.reset()
  ...
  def step(self, a):
    ...
    p_prev = self.p
    self.p = np.clip(self.p + clipped_a, -9, 9)
    idx = self.idx
