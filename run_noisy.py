...
def argparser(args=None):
  ...
  add('--expr_dir', default='expr_noisy')
  add('--pi_step', type=int, default=1000)
  add('--d_step', type=int, default=1000)
  add('--alg', type=str, choices=['sac', 'gail', 'dac', 'airl',
  ...
  add('--noise_lvl', type=float, default=1e-3)
...
def get_expr_name(args):
  ...
  expr_name += f'.noise_{sci(args.noise_lvl)}'
...
def main(args):
  ...
  if args.alg == 'sac':
    ...
    expert_traj_path = f'data_noisy/trj{num_demo}.{env_name}.{args.noise_lvl}.npz'
